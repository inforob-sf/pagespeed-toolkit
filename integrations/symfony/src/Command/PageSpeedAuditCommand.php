...
#[AsCommand(
    ...
    description: 'Audit all project pages with Google PageSpeed Insights',
)]
class PageSpeedAuditCommand extends Command
{
    ...
    {
        $this
            ->addOption('strategy', 's', InputOption::VALUE_OPTIONAL, 'mobile|desktop|both', 'both')
            ->addOption('output', 'o', InputOption::VALUE_OPTIONAL, 'Path to JSON report', 'var/pagespeed-report.json')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $report     = [];
        $outputFile = $this->projectDir . '/' . ltrim($input->getOption('output'), '/');
    ...
    private function resolvePages(InputInterface $input): array
    {
        ...
        // Default: read from PAGESPEED_URLS env var (comma-separated) or fall back to common routes
        ...
        // Generic fallback — commonly audited pages
        return [
            '/'        => 'Home',
            '/blog'    => 'Blog — index',
            '/contact' => 'Contact',
